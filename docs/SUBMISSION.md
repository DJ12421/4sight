...
Foresight helps students and early-career users learn from their decisions. Instead of stopping at an AI conversation, users clarify options, record an expectation and a small experiment, return to the actual outcome, and apply that experience to their next choice. Gemini helps structure a decision brief and suggests tentative patterns with links to the user's selected source entries. Original commitments stay intact, making hindsight visible.

Firebase Authentication identifies each user. Cloud Firestore stores private decision histories and source-linked reflections. The Express backend verifies identity and validates every write and AI response. Google Cloud Run hosts the frontend and API, with the Gemini credential supplied through Secret Manager. A fictional student journey demonstrates the complete loop without pretending example data is a live result.
