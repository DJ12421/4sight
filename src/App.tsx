...
function Workspace({ user, showSample, theme, onToggleTheme }: { user: User; showSample: boolean; theme: Theme; onToggleTheme: () => void }) {
  ...
  function deleteDecision(decision: Decision) {
    if (!window.confirm(`Permanently delete “${decision.title}” and its reviews? This cannot be undone.`)) return;
    void action(async () => {
      ...
      if (!controller.current.signal.aborted) setDecisions(current => current.filter(item => item.id !== decision.id));
    });
  }
