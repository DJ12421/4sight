...
import { ArrowRight, BookOpen, Compass, Feather, LogOut, Moon, Network, Plus, Sparkles, Sun, Trash2 } from 'lucide-react';
...
type WorkspaceView = 'decisions' | 'patterns' | 'legacy' | 'graph' | 'sample';
type Theme = 'light' | 'dark';
const viewPaths: Record<WorkspaceView, string> = { decisions: '/decisions', patterns: '/patterns', legacy: '/journal', graph: '/graph', sample: '/sample' };
...
function Workspace({ user, showSample, theme, onToggleTheme }: { user: User; showSample: boolean; theme: Theme; onToggleTheme: () => void }) {
  ...
  useEffect(() => {
    if (view !== 'legacy' && view !== 'graph') return;
