...
import { ArrowRight, BookOpen, Compass, Layers, LogOut, Moon, Plus, Sparkles, Sun, Trash2 } from 'lucide-react';
import { auth, db, logOut, signInWithGoogle } from './lib/firebase';
import { request } from './lib/workspace';
import { deleteDecisionFromFirestore, syncSampleDecisionsToFirestore } from './lib/storage';
...
function Workspace({ user, showSample, theme, onToggleTheme }: { user: User; showSample: boolean; theme: Theme; onToggleTheme: () => void }) {
  ...
  return <div className="app workspace"><header className="workspace-header"><button className="brand" onClick={() => navigate('decisions')}><Compass size={25} />foresight<span className="brand-period">.</span></button><nav aria-label="Main navigation"><button aria-current={view === 'decisions' ? 'page' : undefined} onClick={() => navigate('decisions')}><BookOpen size={16} />Decisions</button><button aria-current={view === 'patterns' ? 'page' : undefined} onClick={() => navigate('patterns')}><Sparkles size={16} />Patterns</button><button aria-current={view === 'legacy' ? 'page' : undefined} onClick={() => navigate('legacy')}><Layers size={16} />Earlier journal</button></nav><div className="account"><span title={user.email || ''}>{user.displayName?.split(' ')[0] || 'Your journal'}</span><ThemeToggle theme={theme} onToggle={onToggleTheme} /><button className="icon-button" title="Sign out" aria-label="Sign out" disabled={actionBusy} onClick={() => { if (canLeave()) void action(async () => { updatePath('/'); await logOut(); }); }}><LogOut size={17} /></button></div></header>
    <main className="content" id="main-content">{error && <div className="alert" role="alert">{error}<button className="text-button" onClick={() => setError('')}>Dismiss</button></div>}
      {editing ? <DecisionEditor key={editing.id} initial={editing} decisions={decisions} uid={user.uid} onBack={() => navigate('decisions')} onDirty={onDirty} onSaved={saved} onStartNext={sourceIds => open({ ...newDecision(), sourceIds })} /> : view === 'patterns' ? <Patterns uid={user.uid} decisions={decisions} report={report} onReport={setReport} onOpen={open} /> : view === 'sample' ? <SampleJourney onBack={() => navigate('decisions')} busy={actionBusy} onCopy={() => action(async () => { await request(user.uid, '/api/sample', {}, controller.current.signal); try { await syncSampleDecisionsToFirestore(user.uid); } catch (e) { console.warn('Sample sync notice:', e); } if (!controller.current.signal.aborted) { navigate('decisions'); setFilter('All'); setSearch(''); } })} /> : view === 'legacy' ? <><div className="page-heading"><div><p className="eyebrow">Preserved from Gemini Reflections</p><h1>Your earlier journal.</h1><p className="muted">Your original entries and conversations from the earlier Gemini Reflections app, kept intact as a read-only archive.</p></div></div>{legacyLoading ? <p role="status">Loading earlier entries…</p> : !legacy.length ? <div className="empty-state"><Layers size={40} strokeWidth={1} /><h2>No earlier entries found in archive.</h2><p>The <strong>Earlier journal</strong> tab is reserved for entries preserved from the previous Gemini Reflections app. All new decisions you create are stored in the <strong>Decisions</strong> tab.{decisions.length > 0 ? ` You currently have ${decisions.length} decision${decisions.length === 1 ? '' : 's'} active in your journal.` : ''}</p><button onClick={() => navigate('decisions')}><BookOpen size={16} /> Go to your decisions <ArrowRight size={16} /></button></div> : legacy.map(entry => <article className="paper legacy-entry" key={entry.id}><p className="eyebrow">{entry.mode} · {new Date(entry.createdAt).toLocaleDateString()}</p><h2>{entry.title}</h2><p className="preserve">{entry.prompt}</p><div className="ai-note"><ReactMarkdown>{entry.response}</ReactMarkdown></div>{entry.turns?.map((turn, i) => <div className="legacy-turn" key={i}><strong>{turn.role === 'user' ? 'You' : 'Gemini'}</strong><ReactMarkdown>{turn.text}</ReactMarkdown></div>)}</article>)}{legacy.length >= legacySize && <button className="secondary" onClick={() => setLegacySize(size => size + 100)}>Load older entries</button>}</> : <>
