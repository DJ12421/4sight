...
import { ArrowRight, BookOpen, CalendarDays, ChevronLeft, ChevronRight, Download, Feather, FileJson, MessageCircle, Mic, MicOff, Search, ShieldCheck, Sparkles, Tags, Trash2, X } from 'lucide-react';
...
export function Journal({ uid, entries, decisions, loading, canLoadOlder, onDirty, onSaved, onDeleted, onDeleteAll, onStartDecision, onLoadOlder }: {
  ...
  onStartDecision: (entry: JournalInteraction) => void; onLoadOlder: () => void;
}) {
  ...
  useEffect(() => { onDirty(!!busy || listening || !!title || !!entry || !!tags || !!followUp || !!tagEditorId); return () => onDirty(false); }, [busy, listening, title, entry, tags, followUp, tagEditorId, onDirty]);
  useEffect(() => () => voice.current?.abort(), []);
  const allTags = useMemo(() => [...new Set(entries.flatMap(item => item.tags || []))].sort(), [entries]);
  const filtered = useMemo(() => { const needle = search.trim().toLowerCase(); return entries.filter(item => (!date || dayKey(item.createdAt) === date) && (!tag || item.tags?.includes(tag)) && (!needle || [item.title, item.prompt, item.response, ...(item.tags || []), ...(item.turns || []).map(turn => turn.text)].join(' ').toLowerCase().includes(needle))); }, [entries, search, tag, date]);
  ...
  async function submit() {
    ...
    try { const saved = await request<JournalInteraction>(uid, '/api/journal', { id: crypto.randomUUID(), title, entry, mode, tags: parsedTags }); onSaved(saved); setTitle(''); setEntry(''); setTags(''); }
  ...
  async function continueEntry(item: JournalInteraction) {
    ...
    try { const saved = await request<JournalInteraction>(uid, '/api/journal', { id: item.id, message: followUp }); onSaved(saved); setFollowUp(''); setActiveId(''); }
  ...
  async function deleteEntry(item: JournalInteraction) {
    ...
    try { await request(uid, `/api/journal/${item.id}`, undefined, undefined, 'DELETE'); onDeleted(item.id); }
  ...
  async function saveTags(item: JournalInteraction) {
    ...
    try { const saved = await request<JournalInteraction>(uid, `/api/journal/${item.id}/tags`, { tags: parsed }, undefined, 'PUT'); onSaved(saved); setTagEditorId(''); setEditTags(''); }
  ...
  async function exportAccount(format: 'markdown' | 'json') {
    ...
    try { const data = await request<ExportData>(uid, '/api/export', undefined, undefined, 'GET'); download(format === 'markdown' ? 'foresight-journal.md' : 'foresight-data.json', format === 'markdown' ? 'text/markdown' : 'application/json', format === 'markdown' ? journalMarkdown(data) : JSON.stringify(data, null, 2)); }
    catch (e) { setError(e instanceof Error ? e.message : 'Could not prepare your export.'); } finally { setBusy(''); }
  }
  async function deleteAll() {
    if (window.prompt('Type DELETE to permanently remove every journal entry, decision, review, pattern, and usage record. Your sign-in account will remain.') !== 'DELETE') return; setBusy('delete-all'); setError('');
    try { await request(uid, '/api/account-data', undefined, undefined, 'DELETE'); onDeleteAll(); }
    catch (e) { setError(e instanceof Error ? e.message : 'Could not delete your stored data.'); } finally { setBusy(''); }
  }
  ...
  return <>
    ...
    <div className="journal-tools"><label className="journal-search"><span className="sr-only">Search journal</span><Search size={16} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search your journal" /></label><div className="tag-filters"><button aria-pressed={!tag} onClick={() => setTag('')}>All tags</button>{allTags.map(value => <button key={value} aria-pressed={tag === value} onClick={() => setTag(tag === value ? '' : value)}>#{value}</button>)}</div></div>
    <JournalCalendar entries={entries} decisions={decisions} selected={date} onSelect={setDate} />
    <div className="journal-history-heading"><div><p className="eyebrow">Pages and conversations</p><h2>{search || tag || date ? `${filtered.length} matching ${filtered.length === 1 ? 'page' : 'pages'}` : 'Your reflections'}</h2></div><span className="small muted">Private to your account</span></div>
    {loading ? <p role="status">Opening your journal…</p> : !filtered.length ? <div className="empty-state"><Feather size={40} strokeWidth={1} /><h2>{entries.length ? 'No pages match this view.' : 'Your first page is blank.'}</h2><p>{entries.length ? 'Try another search, tag, or calendar date.' : 'Write above. Your words remain the record; Gemini’s response is kept clearly separate.'}</p></div> : filtered.map(item => <article className="paper journal-entry" key={item.id}>
      <div className="journal-entry-meta"><div><span className="tag">{modeLabels[item.mode as Exclude<ReflectionMode, 'chat'>] || 'Earlier reflection'}</span>{item.tags?.map(value => <button className="entry-tag" key={value} onClick={() => setTag(value)}>#{value}</button>)}</div><time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</time></div>
      <h2>{item.title}</h2><p className="preserve journal-words">{item.prompt}</p><div className="ai-note"><span className="eyebrow">Gemini · a reflection</span><ReactMarkdown>{item.response}</ReactMarkdown></div>
      ...
      {tagEditorId === item.id ? <div className="journal-tag-editor"><label>Edit tags <span className="muted small">(comma-separated)</span><input autoFocus maxLength={248} value={editTags} onChange={event => setEditTags(event.target.value)} /></label><div className="button-row"><button onClick={() => saveTags(item)} disabled={!!busy}><Tags size={15} />Save tags</button><button className="text-button" onClick={() => { setTagEditorId(''); setEditTags(''); }} disabled={!!busy}>Cancel</button></div></div> : activeId === item.id ? <div className="journal-follow-up"><label>Continue this reflection<textarea rows={3} maxLength={4000} autoFocus value={followUp} onChange={event => setFollowUp(event.target.value)} placeholder="What feels true, unclear, or different now?" disabled={!!busy} /></label><div className="button-row"><button onClick={() => continueEntry(item)} disabled={!!busy || !followUp.trim()}><MessageCircle size={15} />{busy === item.id ? 'Reflecting…' : 'Send to Gemini'}</button><button className="text-button" onClick={() => { setActiveId(''); setFollowUp(''); }} disabled={!!busy}>Cancel</button></div></div> : <div className="journal-entry-actions"><button className="secondary" onClick={() => { setActiveId(item.id); setFollowUp(''); }} disabled={!!busy}><MessageCircle size={15} />Continue reflecting</button><button className="text-button" onClick={() => { setTagEditorId(item.id); setEditTags((item.tags || []).join(', ')); }} disabled={!!busy}><Tags size={15} />Edit tags</button><button className="text-button" onClick={() => onStartDecision(item)} disabled={!!busy}><BookOpen size={15} />Turn this into a decision <ArrowRight size={15} /></button><button className="icon-button journal-delete" aria-label={`Delete ${item.title}`} title="Delete entry" onClick={() => deleteEntry(item)} disabled={!!busy}><Trash2 size={16} /></button></div>}
    </article>)}
    {canLoadOlder && <button className="secondary" onClick={onLoadOlder}>Load older entries</button>}
    <section className="paper privacy-panel"><div><ShieldCheck size={22} /><div><p className="eyebrow">Your data</p><h2>Keep a copy—or clear the slate.</h2><p>Exports come directly from your authenticated Firestore records. Deleting all stored data keeps your Firebase sign-in account so you can start again.</p></div></div><div className="privacy-actions"><button className="secondary" onClick={() => exportAccount('markdown')} disabled={!!busy}><Download size={15} />Export journal</button><button className="secondary" onClick={() => exportAccount('json')} disabled={!!busy}><FileJson size={15} />Export all data</button><button className="danger-button" onClick={deleteAll} disabled={!!busy}><Trash2 size={15} />Delete all stored data</button></div></section>
  </>;
}
